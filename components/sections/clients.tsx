    ...
    <Section className="border-t border-border">
      <Container>
        ...
        <div
          ...
          className="mt-10 flex gap-6 overflow-x-auto md:hidden"
          style={{ scrollbarWidth: "none" }}
        >
          {logos.map(({ name, logo, w, h, url }, i) => (
            <a
              key={`${name}-${i}`}
              href={url}
              target="_blank"
              ...
              onClick={handleStop}
              className="flex-[0_0_auto] group flex items-center justify-center rounded-lg border border-border-subtle bg-white/[0.04] px-8 py-4 backdrop-blur-sm transition-all duration-500 hover:border-white hover:bg-white/[0.08]"
            >
              <Image
                src={logo}
                alt={name}
        ...
        <Marquee
          pauseOnHover
          repeat={6}
          className="mt-10 hidden md:flex [--duration:30s] [--gap:4rem]"
        >
